...
def classify_body_type(shoulder_w, waist_w, hip_w):
    ...
    if 0.85 <= sh <= 1.15 and wh < 0.82 and ws < 0.82:
        ...
        tips  = ["Fitted dresses and wrap styles look great on you.", "Highlight your waist with belts.", "Almost every silhouette works — own it!"]
        color = "#a855f7"
        emoji = "⌛"
    ...
    # Pear: hips clearly wider than shoulders
        ...
        tips  = ["A-line and flared skirts balance your proportions.", "Boat necks and off-shoulder tops add width on top.", "Avoid clingy fabrics around the hips."]
        color = "#ec4899"
        emoji = "🔻"
    ...
    # Inverted Triangle: shoulders clearly wider than hips
        ...
        tips  = ["Wide-leg trousers and flared jeans add balance.", "V-necks draw the eye inward.", "Avoid heavy shoulder details."]
        color = "#3b82f6"
        emoji = "🔺"
    ...
    # Rectangle: all three roughly the same
        ...
        tips  = ["Peplum tops and ruffles create curves.", "Belted outfits define your waist.", "Layering adds dimension to your look."]
        color = "#10b981"
        emoji = "▬"
    ...
    # Apple: wide waist relative to hips and shoulders
        ...
        tips  = ["Empire-waist and A-line dresses are very flattering.", "Monochrome outfits create a lean look.", "V-necks elongate the neckline beautifully."]
        color = "#f59e0b"
        emoji = "🟡"

    return {
        "shape": shape, "description": desc, "tips": tips,
...
async def detect_body_type(person_image: UploadFile = File(...)):
    ...
    if not results.pose_landmarks:
        raise HTTPException(400, "Could not detect a person in the image. Please use a clear full-body photo.")
    ...
    print(f"Pixel widths → shoulder={shoulder_w:.3f}  waist={waist_w:.3f}  hip={hip_w:.3f}")
...
async def try_on(
    ...
    """
    print(f"Received: person={person_image.filename} ({person_image.content_type}), garment={garment_image.filename} ({garment_image.content_type})")
